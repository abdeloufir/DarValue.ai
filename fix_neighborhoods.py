"""
Clean and properly populate neighborhoods from Mubawab pages
"""
import re
import requests
...
# List of bad neighborhood values that we added by mistake
bad_neighborhoods = [
    'appartement', 'villa', 'vente', 'bel', 'maison', 'superbe', 'magnifique',
    'studio', 'vend', 'belle', 'duplex', 'somptueuse', 'terrasse', 'chambre',
    'appt', 'sidi', 'ain', 'alsace', 'soleil', 'plein', 'boulevard', 'face',
    'appartements', 'villas', 'maisons', 'studios', 'duplexes'
]

# Get all listings with potentially bad neighborhoods
...
print(f"Found {len(bad_listings)} listings with potentially bad neighborhoods")
...
for i, (listing_id, url, current_neighborhood) in enumerate(bad_listings):
    ...
    if neighborhood:
        ...
        print(f"  ID {listing_id}: {current_neighborhood} -> {neighborhood}")
